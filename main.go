...
func cluster() {
	...
	bashCommand := fmt.Sprintf(`aws rds describe-db-instances --region %v | jq -r '.DBInstances[] | { (.DBInstanceIdentifier):(.Endpoint.Address + ":" + (.Endpoint.Port|tostring))}' | sed -e 's/[{}]//g' -e 's/\"//g' -e '/^$/d'`, dbRegion)
...
func execute() {
	...
	bashCommand := fmt.Sprintf(`aws rds describe-db-log-files --region %v --db-instance-identifier %v | jq '.DescribeDBLogFiles[] | select(.LogFileName|test("%v.")) | .LogFileName' | sed -e 's/\"//g'`, dbRegion, dbInstanceIdentifier, logType)
	fmt.Printf("Executing command:\n%v\n\n", bashCommand)
	out, err := exec.Command("bash", "-c", bashCommand).Output()
	if err != nil {
		fmt.Printf("%v", err)
